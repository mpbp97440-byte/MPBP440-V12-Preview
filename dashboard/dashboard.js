...
async function loadDashboard(){
  const data=await loadJson("/MPBP440-V12-Preview/data/dashboard-app.json?v=6.1.1",{});
  if(data.hero){
  ...
  featuredRelease.innerHTML=`${f.cover?`<img src="/MPBP440-V12-Preview/${f.cover}" onerror="this.style.display='none'">`:""}<div><h3>${f.title||"MPBP440"}</h3><p>${f.artist||""}</p><p>${f.date||""}</p><a class="btn ghost" href="${f.url||"/MPBP440-V12-Preview/music/index.html"}">Ouvrir</a></div>`;
