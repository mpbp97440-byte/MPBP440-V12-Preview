...
async function loadData(){
  const data = await fetch("data.json?v=3.0-premium").then(r=>r.json());
  ...
  document.getElementById("upcomingGrid").innerHTML=(data.upcoming||[]).map((x,i)=>`
    <article class="time-card">
      ...
      <div class="time-body"><p class="sup">Étape ${i+1}</p><h3>${x.title}</h3><p><strong>${x.date||""}</strong></p><p>${x.description||""}</p></div>
    </article>`).join("");

  document.getElementById("eventsGrid").innerHTML=(data.events||[]).map(e=>`
    <article class="event-card panel">
      <img src="${e.cover}" alt="${e.title}">
      <div><p class="sup">${e.date||""}${e.time?" • "+e.time:""}</p><h3>${e.title}</h3><p><strong>${e.place||""}</strong></p><p>${e.description||""}</p><a class="btn primary" href="${e.url||"#"}" target="_blank" rel="noopener">${e.buttonText||"Voir l’évènement"}</a></div>
    </article>`).join("");
  ...
  document.getElementById("searchInput")?.addEventListener("input", e => {
    ...
    const filtered = allTracks.filter(t => (t.title || "").toLowerCase().includes(q) || (t.description || "").toLowerCase().includes(q));
...
function renderTracks(tracks, linksHtml){
  document.getElementById("tracks").innerHTML=tracks.map(t=>`
    <article class="card">
      ...
      <div class="card-body">${t.year?`<p class="sup">${t.year}</p>`:""}<h3>${t.title}</h3><p>${t.description||""}</p><div class="platforms">${linksHtml(t.links||{})}</div></div>
    </article>`).join("");
}
