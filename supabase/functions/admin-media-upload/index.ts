...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!(file instanceof File) || !folders.has(folder) || !mediaTypes.has(file.type) || file.size < 1 || file.size > maxBytes) {
      return response({ error: 'invalid media upload' }, 400, request);
    }
    ...
    return response({ path: name, url: data.publicUrl, type: file.type, size: file.size }, 200, request);
  } catch (error) {
    return response({ error: error instanceof Error ? error.message : 'upload failed' }, /admin|session|authorization/i.test(String(error)) ? 401 : 500, request);
  }
});
