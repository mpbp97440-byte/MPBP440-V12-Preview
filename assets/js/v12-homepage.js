(() => {
  ...
  const render = async () => {
    try {
      const [data, news, events, gallery] = await Promise.all([fetch(new URL("data.json?v=makeda-sparetdee-20260808", document.baseURI), {cache:"no-store"}).then(r => r.json()), fetch(new URL("data/news.json?v=makeda-sparetdee-20260808", document.baseURI), {cache:"no-store"}).then(r => r.json()), fetch(new URL("data/events.json", document.baseURI), {cache:"no-store"}).then(r => r.json()), fetch(new URL("data/gallery.json", document.baseURI), {cache:"no-store"}).then(r => r.json())]);
      ...
      const upcoming = (data.upcoming || []).filter(item => item.status !== "Disponible");
